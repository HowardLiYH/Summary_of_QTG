...
def TR(CLOSE, PREV_CLOSE, HIGH, LOW):  # True Range
    return MAX(MAX((HIGH - LOW), ABS(PREV_CLOSE - HIGH)), ABS(PREV_CLOSE - LOW))


def IDATR(OPEN, CLOSE, N=20):  # 在ATR 基础上把 High low 用对应的 prev close 和 open 做替代。
    _prev_close = REF(CLOSE, 1)
    IDTR = MAX(ABS(CLOSE - OPEN), ABS(_prev_close - CLOSE))
    return MA(IDTR, N)


def IDTR(OPEN, CLOSE, PREV_CLOSE):  # intraday true Range
    return MAX(ABS(CLOSE - OPEN), ABS(PREV_CLOSE - CLOSE))
...
def PKS(HIGH, LOW, N=22):  # Parkingson number
    H2L = HIGH / LOW
    LNH2L = LN(H2L)
    return SQRT(1/(4*N*LN(2)) * SUM(LNH2L * LNH2L, N))


def O2CPKS(OPEN, CLOSE, N=22):  # 忽略日内high low的Parkingson number
    H2L = MAX(OPEN, CLOSE) / MIN(OPEN, CLOSE)
    LNH2L = LN(H2L)
    return SQRT(1/(4*N*LN(2)) * SUM(LNH2L * LNH2L, N))


def C2CPKS(CLOSE, PREV_CLOSE, N=22):  # 忽略日内high low,关注跨日C2C的Parkingson number
    H2L = MAX(PREV_CLOSE, CLOSE) / MIN(PREV_CLOSE, CLOSE)
    LNH2L = LN(H2L)
    return SQRT(1 / (4 * N * LN(2)) * SUM(LNH2L * LNH2L, N))
