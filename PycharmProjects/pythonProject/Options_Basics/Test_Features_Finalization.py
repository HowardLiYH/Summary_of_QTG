...
def create_prices(old, correspond_index):
    ...
    for date in date_range:
        ...
        for num in range(len(list_of_expiries)):
            # todo: FOR GRAPH ONLY WILL Delete later
            if num != 0:
                continue
            ...
            # ttm = (pd.to_datetime(list_of_expiries[num]) - pd.to_datetime(date)).days
            ud_prc = ula_price
            r = 0
            div = 0
            c_type = 'C'
            ...
            #     # print(f'Today is {date} with due date {list_of_expiries[num]}')
...
if __name__ == '__main__':
    ...
    # Skew_25, Skew_10, Kur_25, Kur_10, A = create_prices(k, test)
